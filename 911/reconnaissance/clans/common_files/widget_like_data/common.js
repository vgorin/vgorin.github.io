      ...
      try {
        ...
        switch(r.ok) {
         ...
         case -4:
          ...
          case -3:
            var to;
            var iframe = ce('iframe', {src: "http://login.vk.com/"}, {visibility: 'hidden', position: 'absolute'});
...
function MessageBox(options) {
  ...
  if (!transparentBG) {
    transparentBG = ce('div', {id: 'popupTransparentBG', className: 'popup_transparent_bg', innerHTML: '<iframe class="box_frame"></iframe>'}, {display: 'none', height: getSize(document)[1]});
  ...
  function refreshCoords() {
    ...
    boxFrame.style.top = boxContainer.style.top = top + 'px';
...
function goAway(lnk, prms) {
  ...
  if (/http:\/\/(www\.)?(regnum\.ru)(\/|$)/i.test(lnk)) {
    window.open(lnk, '_blank');
    return false;
  }
